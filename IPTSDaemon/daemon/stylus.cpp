...
static std::tuple<i32, i32> get_tilt(u32 altitude, u32 azimuth)
{
	...
	const f64 alt = static_cast<f64>(altitude) / 18000 * math::num<f64>::pi;
	const f64 azm = static_cast<f64>(azimuth) / 18000 * math::num<f64>::pi;
	...
	const i32 tx =
		9000 - gsl::narrow<i32>(std::round(atan_x * 4500 / (math::num<f64>::pi / 4)));

	const i32 ty =
		gsl::narrow<i32>(std::round(atan_y * 4500 / (math::num<f64>::pi / 4))) - 9000;
...
void iptsd_stylus_input(Context &ctx, const ipts::StylusData &data, IPTSHIDReport &report)
{
	...
	if (data.proximity) {
		...
		const f64 x = (static_cast<f64>(data.x) / IPTS_MAX_X) * ctx.config.width;
		const f64 y = (static_cast<f64>(data.y) / IPTS_MAX_Y) * ctx.config.height;
